...
dx,dI = np.loadtxt('/Users/Remponator/Desktop/Praktikum/GitHub/Grundpraktikum/V406_Beugung_am_Spalt/Auswertung/T3.txt', unpack=True)
...
def f (phi,s,b,A,B,C,D,E,Z):
    return A*4*(np.cos((np.pi*s*np.sin(B*phi)/j)))**2*(j/(np.pi*b*np.sin(C*phi)))*(np.sin(C*(np.pi*b*np.sin(D*phi))/j))+Z

params, covariance = curve_fit(f, phi, Im, bounds=([10**(-5),-0.005,0.16,-0.005,-0.005,-0.005,0,0],[5*10**(-3),0.655,0.3,1,1,1,1,0.000000025]))
