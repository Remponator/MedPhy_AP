...
dstr  = 4.5*1e-9 #Dunkelstrom in Ampere

#%% Einzelspalt 1

Pos1, I1 = np.loadtxt('/Users/Remponator/Desktop/Praktikum/GitHub/Grundpraktikum/V406_Beugung_am_Spalt/Auswertung/Einzelspalt1.txt', skiprows=1, unpack=True)
...
popt1, pcov1 = curve_fit(theory, phi, I1, p0 = [np.sqrt(np.max(I1)) / 2e-3, 2e-3])
...
plt.plot(x, theory(x, popt1[0], popt1[1])*1e5, 'k-', label='Ausgleichsrechnung')
...
'''Pos2, I2 = np.loadtxt('/Users/Remponator/Desktop/Praktikum/GitHub/Grundpraktikum/V406_Beugung_am_Spalt/Auswertung/T2.txt', skiprows=1, unpack=True)

I2       = I2*1e-9 - dstr # umrechnen in Ampere
zeta2     = Pos2*1e-3
...
'''Pos3, I3 = np.loadtxt('/Users/Remponator/Desktop/Praktikum/GitHub/Grundpraktikum/V406_Beugung_am_Spalt/Auswertung/T3.txt', skiprows=1, unpack=True)

I3       = I3*1e-9 - dstr # umrechnen in Ampere
...
plt.plot(x, theory(x, *popt2)*1e6*5, 'k-', label='Ausgleichsrechnung Einzelspalt')
plt.plot(phi_2, I2*1e6*5, 'g.', label='Messdaten Einzelspalt')
plt.grid()
x = np.linspace(-0.02, 0.02, 1000)
plt.plot(x, I_ds(x, *popt3)*1e7, 'm-', label='Ausgleichsrechnung Doppelspalt')
plt.plot(phi_3, I3*1e7, 'rx', label='Messdaten Doppelspalt')
plt.xlabel(r'$\varphi \,\, / \,\, \mathrm{rad}$')
plt.ylabel(r'$I \,\, / \,\, \mu\mathrm{A}$')
plt.legend(loc='best')
